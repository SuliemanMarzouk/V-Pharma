        ...
        @if($pharmacyOrders && !$pharmacyOrders->isEmpty())
            ...
            <div class="card my-4">
                <div class="card-body">
                    ...
                    <p class="card-text">Order Total: ${{ $pharmacyOrder->TotalPrice }}</p>
                    <table class="table">
                        <thead>
                            <tr>
                                <th>Medicine Name</th>
                        ...
                        <tbody>
                            @foreach($pharmacyOrder->pharmacyOrderDetails as $orderDetail)
                                <tr>
                                    <td>{{ $orderDetail->medicine->MedicineName }}</td>
                                    <td>${{ $orderDetail->medicine->warehouses->first()->pivot->Price }}</td>
                    ...
                    <div class="d-flex justify-content-between">
                        <form action="/accept-Ph-order/{{ $pharmacyOrder->ID }}" method="POST" style="display: inline;">
                            @csrf
                            <button type="submit" class="btn btn-success">Accept</button>
                        </form>
                        <form action="/reject-Ph-order/{{ $pharmacyOrder->ID }}" method="POST" style="display: inline;">
                            @csrf
